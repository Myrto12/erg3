...
void text_input(char text[words][characters+1]){
    ...
    if(fptr==NULL){
        ...
        printf("The file 'keimeno.txt' was successfully opened\n");
    }
    int text_words = 0;
    ...
    while(go_to_main_menu!=1){
        while(text[i][0]!='\0'){
            ...
            while(vocabulary[j][0]!='\0'){
                int compare = strcmp(text[i],vocabulary[j]);
                i++;
                j++;
                if(compare!=0){
...
void text_statistics(char text[words][characters+1]){
    ...
    printf("\n");
    printf("The statistics will be saved at the file 'statistics.txt'\n\n");
    FILE *stats_ptr;
